...
/* =========================
   CONSTONIC ADMIN V3.5
   穩定修正版：日期放大、批次休假、明確儲存、同步
========================= */
window.CONSTONIC_ADMIN_VERSION = "V3.5";
function v35ToISO(date){const y=date.getFullYear();const m=String(date.getMonth()+1).padStart(2,"0");const d=String(date.getDate()).padStart(2,"0");return `${y}-${m}-${d}`;}
function v35DateLabel(dateStr){if(!dateStr)return "";const date=new Date(dateStr+"T00:00:00");const week=["日","一","二","三","四","五","六"][date.getDay()];return `${date.getFullYear()} / ${String(date.getMonth()+1).padStart(2,"0")} / ${String(date.getDate()).padStart(2,"0")} 星期${week}`;}
function v35ChangeDate(days){const input=document.getElementById("date");if(!input||!input.value)return;const d=new Date(input.value+"T00:00:00");d.setDate(d.getDate()+days);input.value=v35ToISO(d);v35RefreshDateUI();if(typeof renderBookings==="function")renderBookings();if(typeof v33ForceRefreshAll==="function")v33ForceRefreshAll();}
function v35Today(){const input=document.getElementById("date");if(!input)return;input.value=v35ToISO(new Date());v35RefreshDateUI();if(typeof renderBookings==="function")renderBookings();if(typeof v33ForceRefreshAll==="function")v33ForceRefreshAll();}
function v35RefreshDateUI(){const label=document.getElementById("v35DateLabel");const input=document.getElementById("date");if(label&&input)label.textContent=v35DateLabel(input.value);}
function v35EnsureDateToolbar(){const dateInput=document.getElementById("date");if(!dateInput||document.getElementById("v35DateToolbar"))return;const toolbar=document.createElement("div");toolbar.id="v35DateToolbar";toolbar.className="v35-date-toolbar";toolbar.innerHTML=`<button type="button" onclick="v35ChangeDate(-1)">◀ 前一天</button><button type="button" onclick="v35Today()">今天</button><button type="button" onclick="v35ChangeDate(1)">下一天 ▶</button><div id="v35DateLabel" class="v35-date-label"></div>`;dateInput.parentElement?.appendChild(toolbar);dateInput.classList.add("v35-date-input");v35RefreshDateUI();dateInput.addEventListener("change",v35RefreshDateUI);}
function v35DateRange(start,end){const dates=[];if(!start||!end)return dates;const s=new Date(start+"T00:00:00");const e=new Date(end+"T00:00:00");for(let d=new Date(s);d<=e;d.setDate(d.getDate()+1)){dates.push(v35ToISO(d));}return dates;}
function v35SplitDates(text){return String(text||"").split(/[\n,，、\s]+/).map(s=>s.trim()).filter(Boolean);}
function v35EnsureBlocksPanelV2(){const old=document.getElementById("bookingBlocksCard");if(old)old.remove();const adminMain=document.getElementById("adminMain");if(!adminMain||document.getElementById("bookingBlocksCardV35"))return;const card=document.createElement("section");card.className="card";card.id="bookingBlocksCardV35";card.innerHTML=`<h2>店休／個人休假</h2><p class="hint">可用連續日期，也可輸入分開日期。全店店休選「全店」，個人休假選美容師。</p><div class="v35-tabs-lite"><label><input type="radio" name="v35BlockMode" value="range" checked> 連續日期</label><label><input type="radio" name="v35BlockMode" value="multi"> 分開日期</label></div><div class="form-grid"><div class="field v35-range-field"><label>開始日期</label><input type="date" id="v35BlockStart" value="${document.getElementById("date")?.value||v35ToISO(new Date())}"></div><div class="field v35-range-field"><label>結束日期</label><input type="date" id="v35BlockEnd" value="${document.getElementById("date")?.value||v35ToISO(new Date())}"></div><div class="field v35-multi-field hidden"><label>分開日期</label><textarea id="v35BlockDates" rows="3" placeholder="例如：2026-07-01、2026-07-05、2026-07-12"></textarea></div><div class="field"><label>關閉對象</label><select id="v35BlockTherapist"><option>全店</option><option>雅潔老師</option><option>巧萱美容師</option><option>曼曼美甲師</option></select></div><div class="field"><label>原因</label><input id="v35BlockReason" placeholder="例如：公休、課程日、老師休假"></div></div><button type="button" class="primary" onclick="v35CreateBlocks()">新增休假／店休</button><div id="v35BlocksContent" class="booking-blocks-list muted">載入中...</div>`;adminMain.appendChild(card);card.querySelectorAll('input[name="v35BlockMode"]').forEach(r=>r.addEventListener("change",v35ToggleBlockMode));document.getElementById("v35BlockStart")?.addEventListener("change",v35LoadBlocks);v35ToggleBlockMode();v35LoadBlocks();}
function v35ToggleBlockMode(){const mode=document.querySelector('input[name="v35BlockMode"]:checked')?.value||"range";document.querySelectorAll(".v35-range-field").forEach(el=>el.classList.toggle("hidden",mode!=="range"));document.querySelectorAll(".v35-multi-field").forEach(el=>el.classList.toggle("hidden",mode!=="multi"));}
async function v35CreateBlocks(){const mode=document.querySelector('input[name="v35BlockMode"]:checked')?.value||"range";const therapist=document.getElementById("v35BlockTherapist")?.value||"全店";const reason=document.getElementById("v35BlockReason")?.value||"休假/不開放";let dates=[];if(mode==="range"){dates=v35DateRange(document.getElementById("v35BlockStart")?.value,document.getElementById("v35BlockEnd")?.value);}else{dates=v35SplitDates(document.getElementById("v35BlockDates")?.value);}if(!dates.length){alert("請輸入要關閉的日期");return;}const rows=dates.map(date=>({date,therapist,reason,all_day:true}));const {error}=await db.from("booking_blocks").insert(rows);if(error){alert("新增休假失敗，請確認 V2.1 SQL 已執行成功");console.error(error);return;}alert(`已新增 ${rows.length} 天休假／店休`);v35LoadBlocks();}
async function v35LoadBlocks(){const box=document.getElementById("v35BlocksContent");if(!box)return;const start=document.getElementById("v35BlockStart")?.value||document.getElementById("date")?.value||v35ToISO(new Date());const ym=start.slice(0,7);const year=Number(ym.slice(0,4));const month=Number(ym.slice(5,7));const last=new Date(year,month,0).getDate();const from=`${ym}-01`;const to=`${ym}-${String(last).padStart(2,"0")}`;const {data,error}=await db.from("booking_blocks").select("*").gte("date",from).lte("date",to).order("date",{ascending:true});if(error){box.innerHTML="讀取休假資料失敗";console.error(error);return;}const rows=data||[];if(!rows.length){box.className="booking-blocks-list muted";box.innerHTML=`${ym} 尚無休假／店休設定。`;return;}box.className="booking-blocks-list";box.innerHTML=rows.map(b=>`<div class="block-row"><div><strong>${escapeHtml(b.date)}</strong>｜${escapeHtml(b.therapist||"全店")}｜${escapeHtml(b.reason||"休假/不開放")}</div><button type="button" onclick="v35DeleteBlock('${escapeHtml(b.id)}')">刪除</button></div>`).join("");}
async function v35DeleteBlock(id){if(!confirm("確定刪除這個休假／店休設定？"))return;const {error}=await db.from("booking_blocks").delete().eq("id",id);if(error){alert("刪除失敗");console.error(error);return;}v35LoadBlocks();}
