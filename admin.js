...
/* =========================================================
   CONSTONIC ADMIN V6.0 Final Patch 11
   - 修正「已包含在技術收款中」勾選後，畫面本次實收仍重複加總
   - 即時畫面、儲存資料、月報統一使用同一判斷
========================================================= */
window.CONSTONIC_ADMIN_FINAL_PATCH11="V6.0 Final Patch 11";

function p11CheckboxValue(){
  const boxes=Array.from(document.querySelectorAll('#salesIncludedInTech'));
  const visible=boxes.find(el=>el.offsetParent!==null);
  return Boolean((visible||boxes[boxes.length-1])?.checked);
}
function p11ReceivedTotal(c, includedOverride){
  const method=String(c?.payment_method||"現金");
  const nonCash=["扣儲值","扣課程","團購券","免收款"].includes(method);
  const tech=nonCash?0:p10TechBase(c||{});
  const product=Number(c?.product_amount||0);
  const extra=Number(c?.course_amount||0)+Number(c?.stored_value_new_amount||0);
  const included=typeof includedOverride==="boolean"
    ? includedOverride
    : (typeof c?.sales_included_in_tech==="boolean" ? c.sales_included_in_tech : p10IncludedDefault(c||{}));
  return product+(included?Math.max(tech,extra):tech+extra);
}

/* 讓舊有呼叫也使用 Patch11 計算 */
p10ReceivedTotal=function(c){ return p11ReceivedTotal(c); };

window.c64RecalcAll=function(){
  const c=p10CollectCurrent();
  c.sales_included_in_tech=p11CheckboxValue();
  let tech30=0,tech40=0,fixed=0;
  document.querySelectorAll("#techRows .tech-row,#v2TechRows .v3-tech-row").forEach((row,idx)=>{
    const amount=Number(document.getElementById(`techAmount_${idx}`)?.value||document.getElementById(`v2TechAmount_${idx}`)?.value||0);
    const rate=document.getElementById(`techRate_${idx}`)?.value||document.getElementById(`v2TechRate_${idx}`)?.value||"30";
  ...
  const product=Number(c.product_amount||0);
  const extra=Number(c.course_amount||0)+Number(c.stored_value_new_amount||0);
  const total=p11ReceivedTotal(c,c.sales_included_in_tech);
  document.querySelectorAll("#totalReceived,#v2TotalReceived").forEach(input=>{input.value=total;input.readOnly=true;});
...
/* 捕捉勾選切換，避免舊 inline handler 或重複畫面干擾 */
document.addEventListener('change',function(e){
  if(e.target?.id==='salesIncludedInTech'){
    document.querySelectorAll('#salesIncludedInTech').forEach(box=>{if(box!==e.target)box.checked=e.target.checked;});
    window.c64RecalcAll();
  }
});

const p11OldCollectCheckout=window.collectCheckoutFromForm;
window.collectCheckoutFromForm=function(existing={}){
  const checkout=p11OldCollectCheckout(existing);
  checkout.sales_included_in_tech=p11CheckboxValue();
  checkout.total_received=p11ReceivedTotal(checkout,checkout.sales_included_in_tech);
  checkout.calculated=checkoutTotals(checkout);
...
/* =========================================================
   CONSTONIC V6.0 FINAL PATCH 13
   - 強制將本次實收欄位改為真正可編輯
   - 修正舊模板或重算函式重新加上 readonly / disabled
   - 人工輸入後永遠不被勾選與其他欄位重算覆蓋
========================================================= */
window.CONSTONIC_ADMIN_FINAL_PATCH13="V6.0 Final Patch 13";

function p13UnlockReceivedInput(input){
  if(!input) return;
  input.readOnly=false;
  ...
  input.classList.add('p13-manual-received');

  const field=input.closest('.field');
  const label=field?.querySelector('label');
  if(label && /本次實收/.test(label.textContent||'')) label.textContent='本次實收（人工輸入）';

  if(!input.dataset.p13Bound){
    input.dataset.p13Bound='1';
    const markManual=()=>{
      input.dataset.manual='1';
      input.dataset.p13Manual='1';
    };
    input.addEventListener('beforeinput',markManual,true);
    input.addEventListener('input',markManual,true);
    input.addEventListener('change',markManual,true);
    input.addEventListener('focus',()=>{
      p13UnlockReceivedInput(input);
      try{ input.select(); }catch(_e){}
    },true);
  }
}

function p13UnlockAllReceivedInputs(){
  document.querySelectorAll('#totalReceived,#v2TotalReceived,input[name="total_received"]').forEach(p13UnlockReceivedInput);
}

/* 不論哪一層模板重新繪製，都再次解除鎖定 */
const p13Observer=new MutationObserver(()=>p13UnlockAllReceivedInputs());
p13Observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['readonly','disabled']});

document.addEventListener('focusin',function(e){
  if(e.target?.matches?.('#totalReceived,#v2TotalReceived,input[name="total_received"]')) p13UnlockReceivedInput(e.target);
},true);
...
/* 覆寫最後生效的重算：只更新薪資與建議文字，不寫入人工實收欄位 */
window.c64RecalcAll=function(){
  const c=typeof p10CollectCurrent==='function'?p10CollectCurrent():{};
  c.sales_included_in_tech=typeof p11CheckboxValue==='function'?p11CheckboxValue():Boolean(c.sales_included_in_tech);
  let tech30=0,tech40=0,fixed=0;
  ...
  p13UnlockAllReceivedInputs();
  const inputs=Array.from(document.querySelectorAll('#totalReceived,#v2TotalReceived,input[name="total_received"]'));
  inputs.forEach(input=>{
    /* 僅全新空白欄位第一次帶入建議值；已有數字一律保留 */
    if(input.value==='') input.value=String(suggested||0);
  });

  document.querySelectorAll('#p12SuggestedReceived').forEach(hint=>{
    hint.textContent='系統建議：NT$ '+Number(suggested||0).toLocaleString('zh-TW')+'；實際營收完全以你手動輸入的金額為準。';
  });
  const set=(id,val)=>document.querySelectorAll(`#${id}`).forEach(el=>el.textContent='NT$ '+Number(val||0).toLocaleString('zh-TW'));
  set('salary30',tech30);set('salary40',tech40);set('salaryFixed',fixed);
  set('salaryProduct',Math.round(product*.1));set('salaryCourse',Math.round(extra*.02));
...
/* 套用建議金額是唯一允許系統主動改寫實收的操作 */
window.p12UseSuggestedReceived=function(){
  const value=typeof p12SuggestedTotal==='function'?p12SuggestedTotal():0;
  document.querySelectorAll('#totalReceived,#v2TotalReceived,input[name="total_received"]').forEach(input=>{
    p13UnlockReceivedInput(input);
  ...
  document.querySelectorAll('#p12SuggestedReceived').forEach(hint=>hint.textContent='已套用系統建議：NT$ '+Number(value||0).toLocaleString('zh-TW')+'；仍可直接手動修改。');
};

/* 儲存時直接讀取畫面上的人工數字，最後覆蓋所有舊計算結果 */
const p13OldCollectCheckout=window.collectCheckoutFromForm;
window.collectCheckoutFromForm=function(existing={}){
  p13UnlockAllReceivedInputs();
  const visible=Array.from(document.querySelectorAll('#totalReceived,#v2TotalReceived,input[name="total_received"]')).find(el=>el.offsetParent!==null);
  const input=visible||document.querySelector('#totalReceived,#v2TotalReceived,input[name="total_received"]');
  const manualValue=Math.max(0,Number(input?.value||0));
  const checkout=p13OldCollectCheckout(existing);
  checkout.total_received=manualValue;
  checkout.total_received_manual=true;
  checkout.calculated=checkout.calculated||{};
  checkout.calculated.totalReceived=manualValue;
  return checkout;
};

setTimeout(p13UnlockAllReceivedInputs,0);
setTimeout(p13UnlockAllReceivedInputs,300);
